...
def render(H, W, K, chunk=1024*32, rays=None, c2w=None, ndc=True,
    ...
    Args:
      ...
      use_viewdirs: bool. If True, use viewing direction of a point in space in model. 是否向模型输入视角信息 
    ...
    if use_viewdirs:
        ...
        viewdirs = rays_d # view directions 视角信息 就是 rays_d 的归一化 
...
def batchify_rays(rays_flat, chunk=1024*32, **kwargs):
    ...
    把数量很大的rays分成很多小片段 逐一渲染 然后拼接 防止内存溢出 
    """
    all_ret = {}
...
def render_rays(ray_batch, network_fn, network_query_fn, N_samples,
    ...
    Args:
      ...
      N_samples: int. Number of different times to sample along each ray. 均匀在[near, far]区间均匀采样N_samples个点
    ...
    Returns:
      ...
      z_std: [num_rays]. Standard deviation of distances along ray for each
        sample.
    """
    # 解包rays_batch 分成rays_o, rays_d, near, far
    N_rays = ray_batch.shape[0]
    rays_o, rays_d = ray_batch[:,0:3], ray_batch[:,3:6] # (N_rays, 3) 
    viewdirs = ray_batch[:,-3:] if ray_batch.shape[-1] > 8 else None
    # bounds = torch.reshape(ray_batch[...,6:8], [-1,1,2])
    # near, far = bounds[...,0], bounds[...,1] # [-1,1]
    near, far = ray_batch[..., 6].view(-1, 1), ray_batch[..., 7].view(-1, 1) # (N_rays, 1) 

    # 生成所有射线的均匀采样距离矩阵 z_vals 
    # 其每一行都相同 都是[near, far]区间内均匀分布的N_samples个数 
    ...
    z_vals = z_vals.expand([N_rays, N_samples]) # (N_rays, N_samples) e.g. (1024, 64) 

    if perturb > 0.: # 给采样位置添加扰动 增加鲁棒性 
    ...
    # r(t) = o + td 得到所有的点 一共有N_rays * N_samples个点 
...
def config_parser():
    ...
    parser.add_argument("--multires", type=int, default=10, # 位置的位置编码的L 
                        help='log2 of max freq for positional encoding (3D location)')
    parser.add_argument("--multires_views", type=int, default=4, # 视角的位置编码的L 
...
def train():
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            # Random over all images 从所有图片中选N_rand个ray 
            ...
            # Random from one image 从1张图片中选N_rand个ray 
